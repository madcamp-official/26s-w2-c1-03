...
| 앱/백엔드 스택 | Flutter(Riverpod), NestJS(TypeORM), PostgreSQL(Supabase), Supabase Storage, JWT(Passport), OpenAI API, TourAPI, Kakao 로컬 API |
...
`common`, `config`는 도메인이 아니라 횡단 관심사(공통 응답/에러 포맷, 전역 예외 처리, 환경설정, 외부 API 클라이언트 설정)를 담당합니다. `storage`는 자체 Entity 없이 Supabase Storage 업로드만 담당하는 유틸리티 모듈로, `records` 도메인(Phase 11)에서 처음 필요해집니다.
...
| Storage | Supabase Storage — `record-photos` 버킷(영구, 최종 선택 사진만) / 임시 버퍼는 **로컬 서버 디스크**(API 명세서 §6과 동일) |
...
│
├── storage/                            # Supabase Storage 업로드 유틸(자체 Entity 없음)
...
**User** (`users`) — 닉네임, 프로필 이미지 URL(Supabase Storage에 FE가 직접 업로드 후 URL만 저장), 상태(`active`/`withdrawn`, soft delete)
...
- **구현 내용**:
  ...
  - FE: 최초 로그인 시 닉네임 입력 화면, 프로필 조회/수정 화면, 프로필 이미지는 **Supabase Storage에 직접 업로드**(supabase_flutter) 후 반환된 public URL을 `PATCH /users/me`로 전달, 푸시 권한 요청 + 토큰 등록
...
- **구현 내용**:
  - BE: TourAPI 연동 모듈(`area_code`/`sigungu_code` 기준 조회 → `places` 캐싱) → Kakao 로컬 API 리뷰수/평점 병합 정렬 → `GET /trips/{tripId}/places/candidates` 완성, `GET /places/{placeId}` 상세
  - FE: 후보 리스트(인기순) + 카테고리 선택 시 지도 마커 필터링(후보 목록 범위 내에서 클라이언트가 처리, 별도 재조회 없음)
- **완료 조건**: 후보 목록이 평점·리뷰수 가중치로 정렬되어 반환되고, Kakao 미매칭 장소는 최하위로 정렬됨
- **선행 Phase**: Phase 6
- **산출물**: `places/` 모듈
...
- **구현 내용**:
  ...
  - BE(반드시 이 순서, §6): ① `POST /trips/{tripId}/records`(기록 세션 시작, `(trip_id,user_id)` unique이므로 기존 draft 재사용) ② `.../photos/metadata`(텍스트 메타데이터 배치 등록, `photoRefId` 발급) ③ `.../photos/upload`(1차 필터 통과 사진 실물, 로컬 임시 디스크 pass-through 저장 + TTL cron 삭제, 최대 100장) — ①→②→③ 순서인 이유: metadata로 `photoRefId`를 먼저 발급해야 upload가 그 id에 파일을 매칭할 수 있음 ④ `.../photos/curate`(`taken_at` 기준 일자별 그룹핑 → OpenAI 배치 → 최종 최대 15장, 비추천분 즉시 폐기) ⑤ `.../photos/candidates`(서명 URL 미리보기), `.../photos/finalize`(선택분만 `storage/` 모듈로 Supabase Storage 영구 업로드 + `record_photos` insert, 나머지 전량 폐기)
...
FE가 Supabase Storage에 **직접** 업로드(공개 버킷)하고, 반환된 public URL만 `PATCH /users/me`로 백엔드에 전달합니다. 백엔드는 파일 자체를 다루지 않고 `users.profile_image_url` 문자열만 저장합니다.
...
[온디바이스] 1차 필터링(흔들림/노출/중복 제거, OCR 문서 감지, 얼굴 감지 감점) + EXIF 추출/GPS→지명 변환
        ...
        [사용자] 최종 사용할 사진 선택
                  ↓
        photos/finalize → 선택분만 `storage/` 모듈로 Supabase Storage 암호화 영구 업로드 + record_photos insert
...
| 외부 연동 | HTTP mock(nock/msw) 또는 인터페이스 Mock | TourAPI, Kakao 로컬 API, OpenAI 호출 | 실제 외부 API 비용/불안정성 없이 검증 |
...
- **Docker**: Multi-stage 빌드(Node 빌드 스테이지 → 실행 스테이지)로 이미지 크기 최소화. 환경변수(DB 접속정보, JWT 시크릿, OpenAI Key, Supabase Storage Key)는 이미지에 포함하지 않고 실행 시 주입
- **Supabase**: PostgreSQL 서버리스 + Storage만 사용(Supabase Auth/Realtime은 사용하지 않음 — 자체 JWT로 인증 구현)
...
| 리스크 | Supabase Storage/DB 요금·쿼터 | 무료 티어 한도 확인, 테스트 데이터 정리 주기적 수행 |
| 열린 질문 | TourAPI 장소와 Kakao 로컬 API 리뷰/평점 매칭 로직(Phase 7) | 장소명·좌표 기반 매칭 정확도 확보 방안을 Phase 7 착수 전 확정 |
