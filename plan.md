...
- **완료 조건**: 후보 목록이 **관광지 집중률** 순(정렬 정책 변경분 반영)으로 정렬되어 반환되고, 집중률 미매칭 장소는 뒤로 배치됨 — **BE는 완전히 충족**. FE 완료조건("카테고리 선택 시 지도 마커 필터링")은 **미충족**(리스트 UI로 구현됨, 카테고리 필터는 클라이언트 사이드로 해결됨) — 지도 마커 항목만 팀 결정 대기
- **선행 Phase**: Phase 6
...
### Phase 11 — AI 사진 선별 및 기록 파이프라인 ✅ BE·FE 구현 완료 (실기기 완주 검증만 대기)
- **목표**: 온디바이스 필터부터 최종 저장까지 사진 파이프라인 전 구간 동작 (개인정보 요구사항이 걸린 핵심 Phase)
- **[2026-07-15 감사 반영]** 코드 대조 결과 BE 전 엔드포인트·FE 온디바이스 파이프라인이 모두 구현되어 있어 `[ ]`→`[x]`로 갱신함. BE `npx tsc --noEmit` 통과, 전체 `npx jest` **21 suites / 250 tests 전부 통과**(records.service/photo-buffer-cleanup/photo-preview.controller/curate-day-allocation/photo-preview-token 스위트 포함). FE `flutter analyze` 에러 0(info 24건, 스타일 제안뿐). 남은 것은 **실기기 E2E 1회 완주 + 버퍼 폐기 수동 확인**뿐(아래 완료 조건).
- **FE 체크리스트 (가장 난이도 높음, 먼저 착수)** — `features/records/data/*`로 구현 확인:
  - [x] 온디바이스 1차 필터링 — 흔들림/노출/중복 제거(`photo_quality_scorer.dart`, `duplicate_detector.dart`, `photo_filter_pipeline.dart`)
  - [x] OCR 기반 문서성 사진(여권/신분증/항공권 바코드/신용카드) 자동 제외 — `sensitive_content_detector.dart`(google_mlkit_text_recognition + google_mlkit_barcode_scanning, MRZ/카드번호 텍스트 패턴 + PDF417/Aztec/QR 바코드)
  - [x] 얼굴 감지 기반 제3자 비중 감점 — `face_privacy_scorer.dart`(google_mlkit_face_detection, 면적 비중·얼굴 수로 0~1 감점, 제외 아닌 감점)
  - [x] 1차 필터 통과율을 여행 규모별로 적용 + **전체 상한 100장** 하드캡(`photo_filter_pipeline.dart`)
  - [x] EXIF 추출(촬영일시, GPS) — 기기 내 처리(`exif_location_service.dart`)
  - [x] GPS → 지명 변환(역지오코딩) + **원본 좌표 구조적 파기** — `exif_location_service.dart`가 `PhotoCandidate`(위경도 필드 없음)만 반환해 좌표가 스코프 밖으로 나갈 수 없음(§8.2 충족)
  - [x] 사진첩 접근을 여행 시작일~종료일 범위로 쿼리 제한 — `photo_library_service.dart`의 `queryByDateRange`(종료일 23:59:59 포함)
  - [x] 사진첩 조회는 사용자 액션 시점에만 트리거 — `photo_library_service.dart` 주석 규약(initState 자동 호출 금지, "기록 시작"에서만 호출)
  - [x] 필터링 지연 대비 "필터 없이 최근 N장" 폴백 — 직접 선택 모드(`record_manual_pick_screen.dart`)로 파이프라인 우회 경로 제공
- **BE 체크리스트 (반드시 이 순서, API 명세서 §4)** — `records.controller.ts` 등으로 엔드포인트 전수 확인:
  - [x] ① `POST /trips/{tripId}/records` — 기록 세션 시작, `(trip_id,user_id)` unique로 기존 draft 재사용
  - [x] ② `POST .../records/{recordId}/photos/metadata` — 텍스트 메타데이터만 배치 등록, `photoRefId` 발급(`record_photo_ref` 엔티티, 이 시점 `record_photos` 미기록)
  - [x] ③ `POST .../records/{recordId}/photos/upload` — multipart, `photoRefId`별 매칭, 로컬 임시 디스크 pass-through(`bufferDir`), TTL 부여
  - [x] TTL 강제 삭제 cron 잡 — `photo-buffer-cleanup.service.ts`(`@Cron(EVERY_5_MINUTES)`, mtime+TTL 초과분 unlink + DISCARDED 마킹)
  - [x] ④ `POST .../records/{recordId}/photos/curate` — `takenAt` 일자별 그룹핑 → `allocateDailyQuotas`로 동적 배분(최대 15장) → OpenAI 배치(`stripExif` 이중 스트립 후 전송) → 비추천분 즉시 `discardRef` 폐기. AI 실패 시 최신순 폴백
  - [x] ⑤ `GET .../records/{recordId}/photos/candidates` — 짧은 TTL 서명 토큰 미리보기(`photo-preview-token.util.ts` + `photo-preview.controller.ts`)
  - [x] ⑤ `POST .../records/{recordId}/photos/finalize` — 선택분만 `storage.service` Firebase 영구 업로드 + `record_photos` insert, 미선택분 전량 폐기(이관 후 선택분 임시본도 폐기)
  - [x] `PATCH .../photos/{recordPhotoId}` — 캡션/순서/대표사진(`isCover`) 수정
  - [x] `DELETE .../photos/{recordPhotoId}` — 개별 삭제(스토리지 파일 동반), 대표사진이었으면 `trips.cover_image_url` 자동 해제
  - [ ] `GET .../records/{recordId}/writing-template` — AI 글쓰기 템플릿(선택 기능, §15 드랍 후보) — **미구현(의도된 드랍 후보)**
  - [x] `PATCH .../records/{recordId}` — 일기 본문 작성/수정, `draft`→`published` 전환
  - [x] 비공개 원칙: 모든 엔드포인트가 `record.user_id == 요청자` 검증
  - [x] 보안: OpenAI 전송 이미지 EXIF 재검증(`stripExif` 이중 스트립, `records.service.ts:761`)
  - [x] Service 단위 테스트: curate 일자별 배분(`curate-day-allocation.util.spec.ts`), 임시 버퍼 폐기(`records.service.spec.ts`/`photo-buffer-cleanup.service.spec.ts`) 커버
- **FE 체크리스트 (업로드~선택 UI)**:
  - [x] BE ①~③에 맞춰 업로드 화면 연동(`record_upload_screen.dart`, `photo_upload_service.dart` — 메타데이터 등록 → 실물 업로드 순서 준수)
  - [x] 추천 사진 그리드 뷰(`record_selection_screen.dart`)
  - [x] 최종 선택 화면(선택/해제, 캡션 입력 — `record_selection_screen.dart`/`record_manual_caption_screen.dart`)
- **완료 조건**: 사진 업로드 → curate → 사용자 선택 → finalize까지 실기기로 최소 1회 완주, **임시 버퍼 폐기(curate 비추천분, finalize 미선택분)가 실제로 지워지는지 확인**(안 지워지면 개인정보 요구사항 위반 — 반드시 수동 검증) — 코드/유닛테스트는 충족, **실기기 완주·버퍼 폐기 육안 확인만 남음**
...
### Phase 12 — 여행 기록 관리 & 대표사진 ✅ BE·FE 구현 완료
- **목표**: 작성된 기록을 조회·수정·삭제하고 여행 대표사진을 관리
- **[2026-07-15 감사 반영]** `records-summary.controller.ts`/`trip-cover.controller.ts`/`records.service.ts`로 전 엔드포인트 확인, FE `record_write_screen.dart`/`records_list_screen.dart`/`record_detail_screen.dart` 확인해 `[x]`로 갱신. git 로그의 `feature/phase12-record-trip-picker` 머지들이 이 Phase 산출물. BE 테스트는 Phase 11과 같은 스위트에 포함되어 전부 통과.
- **BE 체크리스트**:
  - [x] `PATCH /trips/{tripId}/records/{recordId}` — 일기 본문(`title`/`content`), `status: draft→published` 전환(Phase 11과 공유하는 `records.controller.ts` PATCH)
  - [x] `GET /records` — **본인이 작성한 기록만** 요약 목록(커서 페이지네이션, `records-summary.controller.ts` + `list-records-query.dto.ts`)
  - [x] `GET /records/{recordId}` — 상세(사진 목록 포함), 작성자 본인만
  - [x] `DELETE /records/{recordId}` — `deleted_at` soft delete + 연결된 `record_photos` 스토리지까지 hard delete(`deleteRecord`)
  - [x] 기록 삭제 시 대표사진이었던 사진이 있으면 `trips.cover_image_url` 자동 해제
  - [x] `PUT /trips/{tripId}/cover` — 대표사진 지정, `recordPhotoId`가 본인 기록 사진인지 검증(`trip-cover.controller.ts` + `set-trip-cover.dto.ts`)
  - [x] `DELETE /trips/{tripId}/cover` — 대표사진 해제
- **FE 체크리스트**:
  - [x] 기록 작성 화면(캡션 + 본문 — `record_write_screen.dart`)
  - [x] 기록 목록/상세/수정/삭제 화면(`records_list_screen.dart`/`records_list_controller.dart`/`record_detail_screen.dart`)
  - [x] 대표사진 지정 UI(`record_trip_picker_screen.dart` 등, 본인 기록 사진만 선택)
- **완료 조건**: 목록/상세/수정/삭제, 대표사진 지정과 삭제 시 자동 해제가 정상 동작 — 코드/테스트 충족(실기기 확인은 Phase 11 완주 시 함께)
- **선행 Phase**: Phase 11
- **산출물**: `records/` 모듈 완성
...
| **TODO(축소)** | **Phase 7 FE 지도 마커 UI 미구현** — ① 카테고리 재조회 문제는 클라이언트 사이드 필터링으로 **해결됨**(Phase 7 체크리스트 참고), ② 지도 마커 UI는 여전히 없고 리스트 UI로 대체된 상태(지도 패키지 미설치) | 남은 결정은 지도 마커를 실제 구현할지 vs "리스트형으로 변경"을 API 명세서·기능명세서에 공식 반영할지 하나뿐 — 팀 논의 |
| 해결됨(Phase 7) | ~~후보 정렬을 Google 평점 가중치(`rating × log10(reviewCount+1)`)로 함~~ | **관광지 집중률(TourAPI `tats-cnctr-rate.client.ts`) 순 정렬로 교체**(성능+공공누리 약관). Google Places는 키워드 검색/상세 리뷰에만 사용. 근거 메모리: `places-sorting-policy` |
