...
- **팀원별 역할:**
  - 이예원 (FrontEnd Dev): Flutter 앱 UI/UX 전반(로그인, 계획 생성/편집, 사진 선별, 기록 작성/관리 화면), 백엔드 API 연동, 온디바이스 필터링(흔들림/노출/중복 제거, OCR, 얼굴 감지, EXIF 추출) 구현
...
- 기능명세서·API 명세서·개발 계획서를 먼저 작성해 디자인과 기능 전체 구성을 잡고 개발을 시작했다.
- AI 사진 선별을 온디바이스 1차 필터링 → 서버 AI 큐레이션 → 사용자 최종 확정 3단계로 나눠서, 각 단계를 독립적으로 테스트·수정할 수 있었다.
- 목업 이미지를 놓고 화면을 반복 비교하면서 다듬어, 탭마다 제각각이던 UI(배경색·앱바·빈 상태 등)를 하나의 패턴으로 통일할 수 있었다.

### Problem — 아쉬웠던 점, 개선이 필요한 것

- OpenAI API 토큰 사용량·비용 관리가 체계적이지 못했다.
- 화면 설계를 기능 구현 전에 충분히 픽스하지 않고 진행하다 보니, 완성 후에도 레이아웃/문구를 여러 번 다시 고치는 일이 잦았다.
- 애플 로그인처럼 계획 단계에는 있었지만 실제로 구현하지 못한 항목이 있었다.

### Try — 다음번에 시도해볼 것

- 기능 구현 전 세부 디자인 와이어프레임을 먼저 확정하고 시작해서 반복 수정을 줄인다.
- 개발 일정을 날짜별로 세분화해서 계획한다.
- 화면 UI 설계와 DB 등 백엔드 구현 간의 연동을 처음부터 함께 고려해 수월하게 구현한다.

### 팀원별 소감

**이예원:**

> 평소 만들어보고 싶었던 여행 스케줄링 앱을 기획부터 구현까지 직접 참여해서 완성할 수 있어 의미있었다. 프론트엔드와 백엔드 연동이 쉽지 않았지만 잘 구현해서 개발했다.

**이지민:**

> 여행앱을 만들고자 생각은 몇 번 했지만 실제로 시도해본적은 없었는데 좋은 경험이 된 것 같다. 실제로 내가 계획을 짜기 귀찮아서 만든 앱인만큼 내가 제일 열심히 쓸 것 같다.
